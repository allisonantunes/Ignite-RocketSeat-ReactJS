...
export function StoreProfileDialog() {
  ...
  function updateManagerdRestaurantCache({
    name,
    description,
  }: StoreProfileSchema) {
  ...
  const { mutateAsync: updateProfileFn } = useMutation({
    ...
    onMutate({ name, description }) {
      const { cached } = updateManagerdRestaurantCache({ name, description })

      return { previousProfile: cached }
    },
    onError(_, __, context) {
      if (context?.previousProfile) {
        updateManagerdRestaurantCache(context.previousProfile)
      }
    },
  })
